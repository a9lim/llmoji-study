...
* between-centroid std on PC1 and PC2 — the same separation diagnostic
  ``plot_v3_pca_valence_arousal`` reports at the deepest layer;
...
``data/cache/v3_<short>_h_mean_all_layers.npz`` per model. Re-run with
...
def _load_v3_layer_tensor(
    short_name: str,
    rebuild: bool = False,
) -> tuple[pd.DataFrame, np.ndarray, list[int]]:
    """Load (df, X3, layer_idxs) for the v3 run of the named model,
    apply the standard kaomoji-start row filter + canonicalization,
    cache to disk."""
    M = MODEL_REGISTRY[short_name]
    if not M.emotional_data_path.exists():
        raise FileNotFoundError(
            f"no v3 data at {M.emotional_data_path}; "
            f"run LLMOJI_MODEL={short_name} python scripts/03_emotional_run.py"
        )
    cache_path = DATA_DIR / "cache" / f"v3_{short_name}_h_mean_all_layers.npz"
...
def main() -> None:
    ...
    for short in candidates:
        ...
        df, X3, layer_idxs = _load_v3_layer_tensor(short, rebuild=rebuild)
        if len(df) == 0:
    ...
    if len(per_model_metrics) >= 2:
        cross_dir = FIGURES_DIR / "local" / "cross_model"
