...
Usage:
    python scripts/54_ensemble_predict.py \\
...
def _load(model: str, prefer_pilot: bool) -> tuple[pd.DataFrame, str]:
    """Load encoder summary by name, honoring full-vs-pilot preference."""
    p = summary_path_for(model, prefer_full=not prefer_pilot)
    if p is None:
        sys.exit(f"missing summary for {model} (tried pilot + full)")
    is_pilot = p.name.endswith("_pilot_summary.tsv")
    return pd.read_csv(
        p, sep="\t", keep_default_na=False, na_values=[""],
    ), "_pilot" if is_pilot else "full"
...
def main() -> None:
    ...
    if args.claude_gt:
        ...
        # NaN-tolerant: harness encoder TSVs (haiku/opus) carry NaN for
        # faces missing from the v3 face union, and `int(NaN)` raises —
        # so coerce via a helper that treats NaN/None as 0.
        def _to_int(v) -> int:
            try:
        ...
        for f in union:
            if f not in base.index:
                continue
            br = base.loc[f]
            total = _to_int(br.get("total_emit_count", 0))
            if total < args.gt_floor:
                continue
            d = {q: _to_int(br.get(f"total_emit_{q}", 0)) for q in QUADRANTS}
            if sum(d.values()) > 0:
                gt_counts[f] = d
    ...
    # variant lives at data/local/.
    cross_dir = DATA_DIR if args.claude_gt else DATA_DIR / "local"
    out_tsv = cross_dir / f"face_likelihood_ensemble_predict{suffix}.tsv"
    ...
    out_md = cross_dir / f"face_likelihood_ensemble_predict{suffix}.md"
    out_md.write_text("\n".join(lines) + "\n")
