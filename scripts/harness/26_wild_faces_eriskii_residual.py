...
Usage:
    ANTHROPIC_API_KEY=... python scripts/harness/26_wild_faces_eriskii_residual.py
    # or, no Haiku labels (useful for dry-runs):
    python scripts/harness/26_wild_faces_eriskii_residual.py --no-haiku
"""
...
def _scatter_pca_3d_html(
    ...
    *,
    ...
    Match style of ``scripts/local/build_per_face_pca_3d.py``: grey
...
def main() -> None:
    ...
    ap.add_argument("--ensemble-tsv",
                    default=str(DATA_DIR / "face_likelihood_ensemble_predict.tsv"))
    ...
    lines.append("")
    lines.append(f"_{today} — generated by `scripts/harness/26_wild_faces_eriskii_residual.py`._")
    lines.append("")
    lines.append("## Setup")
    lines.append("")
    lines.append(
