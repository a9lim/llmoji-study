...
Usage:
  ...
  python scripts/harness/24_haiku_face_quadrant_judgment.py --model opus --gt-only
  # Override model id explicitly:
  HAIKU_MODEL=claude-haiku-4-5 python scripts/harness/24_haiku_face_quadrant_judgment.py
  OPUS_MODEL=claude-opus-4-7  python scripts/harness/24_haiku_face_quadrant_judgment.py --model opus --gt-only

Outputs (paths adapt to ``--model``):
  data/<short>_face_quadrant_judgment.jsonl
  ...
  data/<short>_face_quadrant_judgment_summary.md
...
from llmoji_study.jsd import jsd_quadrant, similarity
...
def _out_paths(short: str) -> tuple[Path, Path]:
    """Return ``(jsonl_path, summary_path)`` for the given model
    shortname. ``haiku`` keeps its current names so existing artifacts
    don't move; new shortnames get parallel ``<short>_*`` paths."""
    return (
        DATA_DIR / f"{short}_face_quadrant_judgment.jsonl",
        DATA_DIR / f"{short}_face_quadrant_judgment_summary.md",
    )
...
def main() -> None:
    ...
    out_path, summary_path = _out_paths(short)
    ...
    if args.gt_only:
        ...
        if len(df) == 0:
            raise SystemExit(
                ...
                "stale; rerun scripts/local/45_build_face_union.py."
            )
    ...
    if other_short is not None:
        other_path, _ = _out_paths(other_short)
