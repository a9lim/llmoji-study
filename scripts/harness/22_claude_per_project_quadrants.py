...
actual Claude emissions. Three resolution modes:
  ...
  - ``--mode gt-priority`` (default): use Claude-GT (the Opus 4.7
    groundtruth runs, ``data/claude-runs/run-*.jsonl``) for any
...
Usage:
  ...
  python scripts/harness/22_claude_per_project_quadrants.py --mode ensemble
  python scripts/harness/22_claude_per_project_quadrants.py --mode gt-only
  python scripts/harness/22_claude_per_project_quadrants.py \\
...
def _load_ensemble_predictions(path: Path) -> dict[str, dict]:
    if not path.exists():
        sys.exit(f"missing {path} — run scripts/local/56_ensemble_predict.py first")
