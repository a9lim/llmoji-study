...
Usage:
    python scripts/53_topk_pooling.py
    python scripts/53_topk_pooling.py --ks 1,2,3,5,all
"""
...
def _discover(prefer_full: bool) -> dict[str, tuple[str, bool]]:
    """Map encoder_short → (path, is_pilot) using the canonical layout."""
    found = discover_parquets(prefer_full)
    out: dict[str, tuple[str, bool]] = {}
    for name, path in found.items():
        is_pilot = path.endswith("_pilot.parquet")
        out[name] = (path, is_pilot)
    return out


def _load_face_meta(floor: int) -> pd.DataFrame:
    """Load per-face emit metadata (total_emit_count + per-quadrant counts)
    from the canonical v3_face_union parquet.

    Pre-2026-05-05 this read from data/face_h_first_gemma.parquet (per-encoder
    cosine input that happened to carry the same metadata columns); after the
    layout refactor + stale-data cleanup, the union at data/v3_face_union.parquet
    is the canonical source for face_meta.
    """
    parq = DATA_DIR / "v3_face_union.parquet"
    if not parq.exists():
        raise SystemExit(
            f"missing {parq} — run scripts/40_face_union.py first"
        )
    df = pd.read_parquet(parq)
    # The union sums per-quadrant counts but doesn't pre-compute total_emit_count
    # in some snapshots; ensure the column exists.
    if "total_emit_count" not in df.columns:
        df["total_emit_count"] = sum(df[f"total_emit_{q}"] for q in QUADRANTS)
...
def main() -> None:
    ...
    if args.claude_gt:
        ...
        # Build per-face GT distribution from the canonical face union's
        # emit-count cols. (Pre-2026-05-05 this came from a per-encoder
        # face_h_first parquet; the union has the same columns and is
        # the cross-platform source of truth post-refactor.)
        parq = DATA_DIR / "v3_face_union.parquet"
        meta_full = pd.read_parquet(parq)
        if "total_emit_count" not in meta_full.columns:
            meta_full["total_emit_count"] = sum(
                meta_full[f"total_emit_{q}"] for q in QUADRANTS
            )
        meta_full = meta_full[
    ...
    # variant lives at data/local/. (.md sibling further down is local-only.)
    cross_dir = DATA_DIR if args.claude_gt else DATA_DIR / "local"
    out_tsv = cross_dir / f"face_likelihood_topk_pooling{suffix}.tsv"
    ...
    # The .md report only lands when --claude-gt is off (per existing logic).
    out_md = DATA_DIR / "local" / "face_likelihood_topk_pooling.md"
    out_md.write_text("\n".join(lines))
