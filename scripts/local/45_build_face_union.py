...
Usage:
  python scripts/local/45_build_face_union.py
  # Subset
  python scripts/local/45_build_face_union.py --models gemma,qwen
  # No Claude pilot
  python scripts/local/45_build_face_union.py --no-claude
  # No wild data
  python scripts/local/45_build_face_union.py --no-wild
"""
...
def _accumulate_local(jsonl_path: Path, by_face: dict, dropped: dict) -> int:
...
def _accumulate_wild(
    ...
    """Walk `data/hf_dataset/contributors/<id>/<bundle>/<provider>.jsonl`
...
def main() -> None:
    ...
    for m in requested:
        ...
        if not M.emotional_data_path.exists():
            ...
            continue
        n = _accumulate_local(M.emotional_data_path, by_face, dropped)
        n_local_total += n
