...
Inputs:
  data/local/{short}/temp1_pilot.jsonl     — T=1.0, 1 seed × 120 prompts
  data/local/{short}/emotional_raw_temp0.7.jsonl  — T=0.7, 8 seeds × 120 prompts (v3 main)
...
def _analyze_model(short: str) -> tuple[list[dict], str]:
    ...
    pilot_path = M.emotional_data_path.parent / "temp1_pilot.jsonl"
    # T=0.7 baseline lives at the _temp0.7 sibling of emotional_raw under
    # the per-model dir; the canonical M.emotional_data_path is reserved
    # for the T=1.0 v3 main rerun. Post-2026-05-05 the per-model layout
    # collapsed both flavors under data/local/<short>/.
    main_path = M.emotional_data_path.parent / "emotional_raw_temp0.7.jsonl"
    print(f"\n=== {short} ===")
...
def main() -> None:
    ...
    out = DATA_DIR / "local" / "temp_smoke_verdict.md"
    lines: list[str] = []
    ...
    for m in MODELS:
        M = MODEL_REGISTRY[m]
        pilot_path = M.emotional_data_path.parent / "temp1_pilot.jsonl"
        # T=0.7 baseline = sibling of T=1.0 emotional_raw under the per-model dir.
        main_path = M.emotional_data_path.parent / "emotional_raw_temp0.7.jsonl"
