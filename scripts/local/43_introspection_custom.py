...
``data/{short}_introspection_raw.jsonl`` filtered to ``intro_none``)
or to each other.

Usage:
    LLMOJI_MODEL=gemma .venv/bin/python scripts/local/43_introspection_custom.py \\
...
Output:
    data/{short}_introspection_custom_{label}.jsonl
    data/hidden/v3_{short}_introspection_custom_{label}/<row_uuid>.npz
...
def main() -> None:
    ...
    M = current_model()
    raw_path = DATA_DIR / f"{M.short_name}_introspection_custom_{label}.jsonl"
