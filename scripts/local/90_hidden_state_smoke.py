...
captures hidden states via the new sidecar path, then validates:
  ...
  (4) MAX_NEW_TOKENS is the post-cutover budget (16). Earlier values
      reference different tlast / mean windows; cross-cutover data is
      not comparable. The gate fails-fast if config drifts.
...
def main() -> None:
    # (4) Budget gate. The 2026-05-02 cutover dropped MAX_NEW_TOKENS
    # 120→16. Pre-cutover and post-cutover tlast / h_mean reference
    # different windows; let the smoke fail loudly if config drifts.
    assert MAX_NEW_TOKENS == 16, (
        f"MAX_NEW_TOKENS={MAX_NEW_TOKENS}; expected 16 (post-2026-05-02 "
        f"cutover). Cross-cutover tlast/h_mean are not comparable."
    )
