...
1. **Procrustes overlay**: takes the multi-scene raw plotly HTML output by
   `31_v3_quadrant_procrustes.py` and extracts only the overlay scene
...
Usage:
    .venv/bin/python scripts/local/wrap_blog_3d_html.py
"""
...
RAW_PROCRUSTES = STUDY_ROOT / "figures" / "local" / "cross_model" / "fig_v3_quadrant_procrustes_3d.html"
RAW_PER_FACE = STUDY_ROOT / "figures" / "local" / "cross_model" / "fig_v3_per_face_pca_3d.html"
...
# Plotly 3D marker symbols used by 31_v3_quadrant_procrustes.py.
MODEL_MARKERS = {
    "gemma":       "circle",
    "qwen":        "diamond",
    "ministral":   "square",
...
def main() -> None:
    ...
    if not RAW_PER_FACE.exists():
        print(f"  missing {RAW_PER_FACE} — generate it first via scripts/local/build_per_face_pca_3d.py")
    else:
