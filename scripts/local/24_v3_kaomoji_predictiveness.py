...
(a) hidden → face / quadrant. Multi-class logistic regression on
    ...
    emitted (or which Russell quadrant the prompt is from). CV is
    **StratifiedGroupKFold keyed on prompt_id** so all 8 seeds of any
    one prompt go to the same fold — the test asks "from prompts the
    classifier never saw, can it recover the label?" Pre-2026-05-03
    this used row-level StratifiedKFold, which let seeds of a prompt
    leak between train and test; with the prompt-deterministic
    h_first probe scores that meant the quadrant classifier was
    essentially memorizing prompts and reading the labels back,
    hitting 1.000 accuracy. The grouped split is the rigorous
    generalization test. Reports top-1 accuracy, macro-F1, per-face
    recall.
...
``figures/local/v3_kaomoji_predictiveness_compare.json``.
"""
...
def _multiclass_classifier_metrics(
    ...
    """K-fold CV on PCA(pca_dim) → l2-logistic. Returns dict with
    overall accuracy, macro-F1, per-class recall, and the
    cross-validated predictions for confusion-matrix work.

    When ``groups`` is provided, uses ``StratifiedGroupKFold`` so
    that all rows sharing a group id (typically ``prompt_id``) land
    in the same fold. With prompt-deterministic h_first scores plus
    8 seeds per prompt, ungrouped CV leaks the prompt label between
    train and test and inflates accuracy — the quadrant classifier
    hit 1.000 pre-grouping. Otherwise falls back to row-level
    StratifiedKFold.
    """
    counts = pd.Series(y).value_counts()
    majority_acc = float(counts.max() / len(y))
    if groups is None:
        # Row-level: n_splits limited by smallest class size.
        n_splits = max(2, min(5, int(counts.min())))
    else:
        # Group-level: n_splits limited by min unique groups per class
        # (StratifiedGroupKFold needs at least n_splits groups per class).
        groups_per_class = (
            pd.DataFrame({"y": y, "g": groups})
            .drop_duplicates()
            .groupby("y").size()
        )
        n_splits = max(2, min(5, int(groups_per_class.min())))
...
def run_model(short_name: str, *, min_n: int = 5, min_groups: int = 3) -> dict[str, Any]:
    ...
    # Face filter: ≥ min_n total rows AND ≥ min_groups unique prompts so
    # StratifiedGroupKFold can hold any prompt out (a face that only ever
    # appears for 1 prompt has no held-out variant to predict).
    face_counts = df.groupby("first_word").agg(
        n_rows=("first_word", "size"),
        n_prompts=("prompt_id", "nunique"),
    )
    keep_faces = face_counts[
        (face_counts["n_rows"] >= min_n) & (face_counts["n_prompts"] >= min_groups)
    ].index
    face_mask = df["first_word"].isin(keep_faces).to_numpy()
    df_face = df[face_mask].reset_index(drop=True)
    X_face = X[face_mask]
    print(f"  filtering to faces with n_rows ≥ {min_n} AND n_prompts ≥ {min_groups}: "
          f"{len(keep_faces)}/{df['first_word'].nunique()} faces, "
          f"{len(df_face)} rows kept")

    print(f"  hidden → face classifier (PCA(50)→l2-LR, "
          f"StratifiedGroupKFold by prompt_id)...")
    face_metrics = _multiclass_classifier_metrics(
        X_face, df_face["first_word"].to_numpy(),
        groups=df_face["prompt_id"].to_numpy(),
    )
    print(f"    n_classes={face_metrics['n_classes']}  "
          f"n_splits={face_metrics['n_splits']}  "
    ...
    print(f"  hidden → quadrant classifier ({df['quadrant'].nunique()}-class, "
          f"StratifiedGroupKFold by prompt_id)...")
    ...
    print(f"    n_classes={quad_metrics['n_classes']}  "
          f"n_splits={quad_metrics['n_splits']}  "
