...
Runs the same metrics as `scripts/24_v3_kaomoji_predictiveness.py` but
...
Outputs:
  data/local/{short}/{introspection|main}_predictiveness_{which}.tsv
...
_S25_PATH = Path(__file__).parent / "24_v3_kaomoji_predictiveness.py"
...
def main() -> None:
    ...
    if custom_label and not use_main:
        custom_path = (
            M.emotional_data_path.parent / f"introspection_custom_{custom_label}.jsonl"
        )
    ...
    tsv_path = M.emotional_data_path.parent / f"{tag}_predictiveness_{which}.tsv"
