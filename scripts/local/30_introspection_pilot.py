...
Output:
  data/local/{short_name}/introspection_raw.jsonl
  data/local/hidden/{short_name}_introspection/<uuid>.npz
...
# JSONL flush cadence — same rationale as 00_emit.py.
...
def _data_paths() -> tuple[Path, str]:
    ...
    M = current_model()
    raw = M.emotional_data_path.parent / "introspection_raw.jsonl"
