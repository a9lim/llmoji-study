...
The v3 main loop (`scripts/local/03_emotional_run.py`) shows ~2.7s of
...
data/phase_timing_smoke/<short>/hidden/<experiment>_phase_timing/, not
the production v3 dirs.

Usage:
    python scripts/local/98_v3_phase_timing_smoke.py [--n N]
...
def _build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--out", type=Path, default=None,
                   help="output dir (default: data/phase_timing_smoke/<short>/)")
...
def main() -> None:
    ...
    out_dir = args.out or (DATA_DIR / "phase_timing_smoke" / M.short_name)
